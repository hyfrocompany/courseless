// Seeded ocean-gradient cover art. Every lesson carries a `coverSeed`, so its cover is
// stable across launches and the library reads as one palette without shipping any imagery.
...
export interface CoverSpec {
  gradient: string
  shape: ShapeKind
  /** 0..1 normalised anchor for the overlay shape */
  cx: number
  cy: number
  scale: number
  rotate: number
...
// The identity gradient: deep ocean floor -> fill blue -> bright surface -> foam.
...
export function coverSpec(seed: number): CoverSpec {
  const r = rng(seed)
  const angle = Math.round(118 + r() * 54) // 118deg .. 172deg
  ...
  const s1 = Math.round(24 + r() * 14) // 24% .. 38%
  const s2 = Math.round(s1 + 20 + r() * 16) // stays ordered, leaves room for the foam stop
...
/** Deterministic seed for lessons that predate coverSeed, or for ad-hoc art. */
